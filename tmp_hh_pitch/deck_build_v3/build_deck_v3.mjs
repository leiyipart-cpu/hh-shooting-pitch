...
const BUILD = `${ROOT}/tmp_hh_pitch/deck_build_v3`;
const OUT = `${ROOT}/output/HH_2027SS_自在天地间_创意提案_V1.pptx`;
...
function chrome(slide, n, dark = false, section = "HELLY HANSEN 27SS") {
  ...
  text(slide, String(n).padStart(2, "0"), 1168, 676, 52, 18, { fontSize: 11, bold: true, color: dark ? "#FFFFFF/75" : C.grey, typeface: FONT_BODY, align: "right" });
}
...
async function main() {
  ...
  // 10 Looks overview
  {
    const s = newSlide(p); s.background.fill = C.fog; title(s, "8 LOOKS · ONE UNIVERSE", "8套Look，是人在天地之间的8种状态", 10, false);
    image(s, A.brief16, 48, 250, 744, 384, "HH 27SS eight-look overview", "contain");
    const groups = [
      ["向海", "01 经典航海联名\n04 航海潮流限定\n07 航海生活方式"],
      ["亲野", "02 徒步1\n03 徒步2\n06 溯溪"],
      ["循野", "05 越野跑\n08 越野跑场景"],
    ...
    groups.forEach((g, i) => {
      const y = 252 + i * 126;
      text(s, g[0], 840, y, 130, 44, { fontSize: 28, bold: true, color: i === 1 ? C.red : C.oliveDark });
      text(s, g[1], 980, y, 220, 78, { fontSize: 18, bold: true, color: C.olive, lineSpacing: 1.2 });
      line(s, 840, y + 94, 360, C.line, 1);
    });
    note(s, ["User-provided HH 27SS Ambassador Shooting Brief, p.18 (8-look list)."]);
  }
  ...
  // 28 outdoor looks
  {
    const s = newSlide(p); s.background.fill = C.paper; title(s, "FIVE WAYS TO MOVE WITH NATURE", "五套户外Look，是五种回应自然的方式", 28, false);
    const imgs = [A.brief30, A.brief31, A.brief32];
    const labels = ["02 / 03 · 徒步", "06 · 溯溪", "05 / 08 · 越野跑"];
    const desc = ["读懂天气与地势", "顺应水流与光线", "让脚步与山野同步"];
    imgs.forEach((im, i) => {
      const x = 48 + i * 396;
      image(s, im, x, 252, 360, 238, `Outdoor look group ${i + 1}`, "cover");
      text(s, labels[i], x, 512, 360, 32, { fontSize: 19, bold: true, color: C.oliveDark });
      text(s, desc[i], x, 556, 360, 54, { fontSize: 22, bold: true, color: i === 2 ? C.red : C.olive });
    });
    note(s, ["User-provided HH Shooting Brief, pp.30–32 (Hiking, Stream, Trail Running looks)."]);
  }
