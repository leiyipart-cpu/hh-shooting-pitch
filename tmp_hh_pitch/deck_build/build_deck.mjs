...
const BUILD = `${ROOT}/tmp_hh_pitch/deck_build`;
...
function footer(slide, page, dark = false) {
  ...
  addLine(slide, M, 688, 120, C.red, 3, `footer-rule-${page}`);
  ...
  addText(slide, String(page).padStart(2, "0"), 1160, 678, 56, 22, {
    fontSize: 12,
    bold: true,
    color,
    typeface: FONT_SANS,
...
async function main() {
  ...
  // 06 — Big idea / logbook
  {
    ...
    addTag(s, "BIG IDEA", M, 62, { width: 126 });
    ...
    addText(s, "一本在 150 周年正式启用的远行记录。\n不是怀旧道具，而是未来每一年都可由真实 Crew 续写的内容平台。", M, 420, 440, 108, { fontSize: 22, color: "#D3DEE2", lineSpacing: 1.28 });
  ...
  // 33 — Why this wins
  {
    const s = p.slides.add();
    s.background.fill = C.paper;
    titleBlock(s, "WHY THIS WINS", "这套方案同时回答品牌、创意与执行", 33, false);
    const claims = [
    ...
    for (let i = 0; i < 3; i++) {
      ...
      addText(s, claims[i][1], 390, y, 770, 56, { fontSize: 24, color: C.sea, lineSpacing: 1.15 });
      addLine(s, 154, y + 74, 1006, C.line, 1);
    }
  ...
  await file.save(`${OUT}/HH150周年_航海志_赢标创意提案.pptx`);
}
